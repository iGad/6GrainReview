namespace FarmApp.DAL.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<FarmApp.DAL.EF.FarmContext>
    {
        ...
        {
            AutomaticMigrationsEnabled = true;//очень плохое решение, т.к. теряется возможность явно следить за структурой БД. Например, программист внес небольшое изменение, не ожидая, что БД изменится, запустил приложение и так и не узнал об этом, что может привести к большим проблемам
